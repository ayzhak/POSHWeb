...
namespace POSHWeb.Services
{
    public class PSFileService
    {
        ...
        public PSFileService(IServiceScopeFactory scopeFactory, HasherService hasherService,
            ILogger<PSFileService> logger, PSScriptValidator psScriptValidator, PSParameterParserService parameterParserService)
        {
        ...
        public void Sync(string directory)
        {
            ...
            files.ForEach((Action<string>)(path =>
            {
                ...
                var matchingFilenames = _dbContext.Script.Where<PSScript>(script => script.FullPath.Equals(path)).ToList<PSScript>();
                var matchingHashes = _dbContext.Script.Where<PSScript>(script => script.Content.Equals(hash)).ToList<PSScript>();
                ...
                if (matchingFilenames.Count == 0 && matchingHashes.Count == 1)
                {
                    ...
                    _dbContext.Update<PSScript>(script);
        ...
        public PSScript Remove(string fullPath)
        {
            ...
            if (_psScriptValidator.IsValidPsFile(fullPath)) throw new FileStillExistException($"Can not remove file {fullPath}, because it still exist.");
            var _dbContext = scope.ServiceProvider.GetRequiredService<DatabaseContext>();
        ...
        public PSScript Rename(string oldFullName, string newFullName)
        {
            ...
            var isNewFileValid = _psScriptValidator.IsValidPsFile(newFullName) && _psScriptValidator.ValidateFile(newFullName);
